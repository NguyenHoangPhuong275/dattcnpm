  ...
  return (
    <section id="travel-news" className="border-t border-[var(--color-border)] bg-[var(--color-bg)] px-4 py-12 sm:px-6 sm:py-16 lg:px-8 xl:px-12">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8">
          <h2 className="font-display text-3xl font-extrabold text-[var(--color-text)]">Cẩm nang du lịch</h2>
          <p className="mt-1 text-sm font-medium text-[var(--color-text-secondary)]">Kinh nghiệm và cảm hứng cho những hành trình sắp tới.</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {articles.map((item) => (
            <Link
              ...
              key={item.slug}
              href={getTravelReferenceHref(item.slug)}
              className="group overflow-hidden rounded-2xl border border-[var(--color-border)] bg-white text-left shadow-sm transition hover:border-[var(--color-primary-dark)] hover:shadow-md cursor-pointer w-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]"
            >
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs text-[var(--color-text-muted)] font-medium">
                  <span>{formatReferenceDate(item.publishedAt)}</span>
