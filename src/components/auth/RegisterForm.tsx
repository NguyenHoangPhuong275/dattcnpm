...
export default function RegisterForm({ onSuccess, onAuthenticated, onToast }: RegisterFormProps) {
  ...
  const validate = () => {
    if (!agreeTerms) {
      setErrors({ agreeTerms: 'Bạn cần xác nhận trước khi tiếp tục' });
      return false;
    }
    ...
    <form className="flex-1 flex flex-col justify-between py-1" onSubmit={handleSendOTP} noValidate>
      <div className="space-y-5">
        ...
        <div className="space-y-1.5">
          <label htmlFor="register-full-name" className="block text-xs font-semibold text-slate-800">Họ và tên</label>
          <input
            id="register-full-name"
            type="text"
            value={fullName}
            onChange={(e) => setFullName(e.target.value)}
            className={`block min-h-[44px] w-full rounded-2xl border bg-white px-3 py-3 text-sm text-slate-900 placeholder-slate-400 transition-all ${errors.fullName ? 'border-red-300' : 'border-slate-200'}`}
        ...
        <div className="space-y-1.5">
          <label htmlFor="register-email" className="block text-xs font-semibold text-slate-800">Email</label>
          <input
            id="register-email"
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className={`block min-h-[44px] w-full rounded-2xl border bg-white px-3 py-3 text-sm text-slate-900 placeholder-slate-400 transition-all ${errors.email ? 'border-red-300' : 'border-slate-200'}`}
        ...
        <div className="space-y-1.5">
          <label htmlFor="register-password" className="block text-xs font-semibold text-slate-800">Mật khẩu</label>
          <div className="relative">
            <input
              ...
              className={`block min-h-[44px] w-full rounded-2xl border bg-white py-3 pr-10 pl-3 text-sm text-slate-900 placeholder-slate-400 transition-all ${errors.password ? 'border-red-300' : 'border-slate-200'}`}
        ...
        <div className="flex items-start gap-2 text-xs py-0.5">
          ...
          />
          <label htmlFor="register-agree-terms" className="text-xs text-slate-700">
