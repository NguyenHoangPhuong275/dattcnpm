...
export default function UserManagement({
  ...
  async function handleDelete(user: AdminUserItem): Promise<void> {
    const confirmed = await confirm({
      ...
      description: `Tài khoản ${user.email} sẽ không thể đăng nhập và các phiên hiện tại sẽ kết thúc.`,
  ...
  return (
    <section className="admin-surface overflow-hidden">
      <div className="border-b border-black/[0.055] p-6 sm:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="text-lg font-extrabold text-[var(--color-text)]">{total.toLocaleString('vi-VN')} tài khoản</h2>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <input
              ...
              className="admin-field min-w-64"
            />
            ...
            <select
              ...
              className="admin-field app-select sm:w-48"
            >
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] text-left text-sm">
          <thead className="bg-[#faf8f9] text-[10px] uppercase tracking-[0.12em] text-[var(--color-text-muted)]">
            <tr>
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {isLoading ? (
              ...
              <tr key={user._id} className="transition hover:bg-[var(--color-primary-lightest)]/45">
                ...
                <td className="px-4 py-4"><span className="rounded-full bg-[var(--color-primary-lightest)] px-2.5 py-1 text-xs font-bold text-[var(--color-primary-darker)]">{user.role === 'ADMIN' ? 'Quản trị viên' : 'Người dùng'}</span></td>
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    <button id={`admin-user-activity-${user._id}`} type="button" onClick={() => onViewActivity(user)} className="rounded-lg border border-[var(--color-border)] px-3 py-1.5 text-xs font-bold text-[var(--color-text-secondary)] transition hover:bg-[var(--color-primary-lightest)]">Xem hoạt động</button>
                    <button
                    ...
                    <button id={`admin-user-delete-${user._id}`} type="button" disabled={actionLoading !== null || user.role === 'ADMIN'} onClick={() => handleDelete(user)} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-bold text-red-700 disabled:opacity-40">Vô hiệu hóa</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className="flex items-center justify-between border-t border-black/[0.055] bg-[#fdfcfc] px-6 py-4 text-sm">
        ...
        <div className="flex gap-2">
          <button id="admin-users-prev" type="button" disabled={page <= 1 || isLoading} onClick={() => onPageChange(page - 1)} className="admin-button-secondary !min-h-9 !px-3">Trang trước</button>
          <button id="admin-users-next" type="button" disabled={page >= totalPages || isLoading} onClick={() => onPageChange(page + 1)} className="admin-button-secondary !min-h-9 !px-3">Trang sau</button>
        </div>
      </div>
    </section>
  );
